# BWA-Groove — Read this before doing anything
...
can clone, browse, and reason about BWA-Groove on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-Groove happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/groove/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-Groove.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-Groove," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-Groove.vst3`
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-Groove. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/groove/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-Groove row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-Groove.vst3` that:
...
```bash
git subtree push --prefix=plugins/groove \
  git@github.com:vmcguire/BWA-Groove.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-Groove.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-Groove._
